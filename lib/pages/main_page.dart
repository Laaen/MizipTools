...
class MainPageState extends State<MainPage> {
  ...
  void initState() {
    ...
    watchForTag(
        currentTag, nfcAdapter, globalLock, context, onTagLost, onTagDetected);
  }
  ...
  Future<void> onTagDetected(
      Lock globalLock, NfcTag tag, NfcAdapter nfcAdapter) async {
    ...
    if (await isMizipTag(tag, nfcAdapter)) {
      currentTag = MizipTag(
          uid: tag.id.toUint8List(), lock: globalLock, nfcAdapter: nfcAdapter);
    } else {
