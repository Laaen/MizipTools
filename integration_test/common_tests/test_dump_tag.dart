...
Future<void> testDumpTagWrongKey(
  WidgetTester tester,
  MockNfcTag mockTag,
) async {
...
Future<void> testDumpTagSuccess(WidgetTester tester, MockNfcTag mockTag) async {
  ...
  await commonDumpTagExec(
    tester,
    mockAdapter,
    "Dump done file : ${mockAdapter.currentTag!.getUid().toUpperCase()}.dump",
  );
}

Future<void> commonDumpTagExec(
  WidgetTester tester,
  ...
  if (mockAdapter.currentTag!.denyAuthList.isEmpty && !disconnectTag) {
    ...
    final file = dir
        .listSync()
        .where((f) => f.path.split("/").last != "uid_save")
        .last;
    expect(
      file.path.split("/").last,
      equals("${mockAdapter.currentTag!.getUid().toUpperCase()}.dump"),
    );

    // Content is OK
    final expectedContent = mockAdapter.currentTag!.data
