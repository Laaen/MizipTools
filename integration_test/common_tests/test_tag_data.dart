...
Future<void> testTagDataNoTag(WidgetTester tester, MockNfcTag? mockTag) async {
  ...
  await commonTagDataExec(tester, mockAdapter, (
    balance: List<Type>.empty(),
    dumps: [ReadDump],
    advanced: List<Type>.empty()
  ), [
    "No tag detected"
  ]);
}

Future<void> testTagDataMifareClassic(
    WidgetTester tester, MockNfcTag mockTag) async {
  final mockAdapter = MockNfcAdapter(tagToSimulate: mockTag);
  mockAdapter.putTag();

  await commonTagDataExec(tester, mockAdapter, (
    balance: List<Type>.empty(),
    dumps: [DumpTag, WriteFromDump, ReadDump],
    advanced: [ChangeUid, AutoRepair]
  ), [
    "UID: ${mockTag.getUid().toUpperCase()}",
...
Future<void> testTagDataMizipIncorrectChecksum(
    WidgetTester tester, MockNfcTag mockTag) async {
  final mockAdapter = MockNfcAdapter(tagToSimulate: mockTag);
  mockAdapter.putTag();
...
Future<void> commonTagDataExec(WidgetTester tester, MockNfcAdapter mockAdapter,
    WidgetsToFind expectedWidgetList, List<String> expectedShownData) async {
  ...
  await checkTab(
      tester, "Balance", expectedWidgetList.balance, expectedShownData);
  await checkTab(tester, "Dumps", expectedWidgetList.dumps, expectedShownData);
  await checkTab(
      tester, "Advanced", expectedWidgetList.advanced, expectedShownData);
}

// Checks if displayed data are correct
Future<void> checkTab(WidgetTester tester, String tabName,
    List<Type> widgetsToFind, List<String> tagDataContent) async {
