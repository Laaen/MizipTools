...
Future<void> testTagDataMizip(WidgetTester tester, MockNfcTag mockTag) async {
  ...
  await commonTagDataExec(
    ...
    ["UID: ${mockTag.getUid().toUpperCase()}", r"Balance: 36.92$"],
  );
}
...
Future<void> commonTagDataExec(
  WidgetTester tester,
  MockNfcAdapter mockAdapter,
  WidgetsToFind expectedWidgetList,
  List<String> expectedShownData,
) async {
  ...
  await checkTab(
    tester,
    "Advanced",
    expectedWidgetList.advanced,
    expectedShownData,
  );
}

// Checks if displayed data are correct
Future<void> checkTab(
  WidgetTester tester,
  String tabName,
  List<Type> widgetsToFind,
  List<String> tagDataContent,
) async {
