...
Future<void> testAdd10Success(WidgetTester tester, MockNfcTag mockTag) async {
  ...
  await commonAdd10Exec(
    tester,
    mockAdapter,
    "Balance changed successfully",
    expectedResultTestAdd10Success,
  );
}

Future<void> testAdd10TagRemoved(
  WidgetTester tester,
  MockNfcTag mockTag,
) async {
  final mockAdapter = MockNfcAdapter(tagToSimulate: mockTag)..putTag();
  ...
  await commonAdd10Exec(
    tester,
    mockAdapter,
    "Error: Could not get tag's current balance : Communication error",
    expectedResult,
    disconnectTag: true,
  );
}

Future<void> commonAdd10Exec(
  WidgetTester tester,
  ...
  await tester.ensureVisible(
    find.widgetWithText(OutlinedButton, r"Add 10$").last,
  );
  await tester.pumpAndSettle(const Duration(seconds: 1));
  if (disconnectTag) {
...
const expectedResultTestAdd10Success = """
