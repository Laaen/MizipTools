...
void main() {
  ...
  group("MiZip tests", () {
    group("Test Data invalid checksum", () {
      testWidgets("Check displayed data in all menus", (tester) async {
        await testTagDataMizipIncorrectChecksum(
            tester, generateMockMizipTagInvalidChecksum());
      });
    });
    ...
    group("Balance", () {
      ...
      testWidgets("Change balance tag removed", (tester) async {
        await testChangeBalanceTagRemoved(
            tester, generateMockMizipTag(), "26.92");
      });
    ...
    group("Dump", () {
      ...
      testWidgets("Read dump success", (tester) async {
        await testReadDumpSuccess(
            tester, generateMockMizipTag(), exampleDumpTestReadDump);
      });
    });

    group("Advanced", () {
      testWidgets("Change UID success", (tester) async {
        await testChangeUid(tester, generateMockMizipTag(), "ABD453C7",
            expectedTagContentChangeUidTestMizipSuccess);
      });

      testWidgets("Change UID not CUID", (tester) async {
        await testChangeUidNotCUID(tester, generateMockMizipTag(), "ABD453C7",
            expectedTagContentChangeUidTestMizipNotCUID);
      });

      testWidgets("Change UID wrong key", (tester) async {
        await testChangeUidWrongKey(
            tester, generateMockMizipTag(), "ABD453C7", "");
      });

      testWidgets("Change UID tag removed", (tester) async {
        await testChangeUidTagRemoved(
            tester, generateMockMizipTag(), "ABD453C7", "");
      });
    });
  });
...
MockNfcTag generateMockMizipTag() {
  return MockNfcTag(type: NfcTagType.mifareClassic, data: [
...
MockNfcTag generateMockMizipTagInvalidChecksum() {
  return MockNfcTag(type: NfcTagType.mifareClassic, data: [
    "ED711B74F3890400C808002000000017".toUint8List(),
    "6200488849884A884B88000000000000".toUint8List(),
    "00000000000000000000000000000000".toUint8List(),
